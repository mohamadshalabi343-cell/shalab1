    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                amount_received = float(amount_received) if amount_received else 0.0
            except ValueError:
                flash('قيمة غير صحيحة في التكلفة أو المبلغ المقبوض', 'error')
            ...
            if not worker_id:
                flash('يرجى اختيار العامل!', 'error')
                return redirect(url_for('add_record'))
